...
% If angle is negative that indicates a G02 cut -> C = C(1,:)
    if sum(strcmp(CNC(i,:),'G02')) == 1 || sum(strcmp(CNC(i,:),'G2')) == 1; %Clockwise circle
    X = C(1,1); Y = C(1,2); tnew = C(1,4); tprev = C(1,5);
    elseif sum(strcmp(CNC(i,:),'G03')) == 1 || sum(strcmp(CNC(i,:),'G3')) == 1;%Counterclockwise circle
...
cosnew = acos((a^2+c^2-b^2)/(2*a*c));
    ...
    if (Cx(e)-Xnew)^2+(Cy(e)-Ynew)^2<=r^2
        in1(e)=in2(e)+1;
    end
    
    if in(e) > 0 || in1(e)>0
        if currentZ(e)>Zvalue
            if sum(strcmp(CNC(i,:),'G02')) == 1 || sum(strcmp(CNC(i,:),'G2')) == 1
